...
- This project does not deploy PHP code to a remote server. `maint/deploy.php` SSHes to the remote MySQL host and applies database schema changes only, then clears local `golive_plan.sql` and refreshes local `schema_dump_latest.sql`.
- When making changes to static CSS or JavaScript files, make sure to increment the cachebuster in `localconfig.php`.
- If a requested change would introduce unnecessary technical debt, hidden coupling, or undue complexity, pause and explain the concern instead of implementing it blindly. Offer a simpler alternative or a staged approach when possible.
